                    ...
                    <table class="table table-striped table-bordered table-hover" id="sample_3">
                        ...
                        @foreach($model as $key=>$tt)
                            <tr>
                                ...
                                @endif
                                <td>
                                    <a href="{{url('danhsachdoituongtx/'.$tt->id.'/show')}}" class="btn btn-default btn-xs mbs"><i class="fa fa-eye"></i>&nbsp;Xem chi tiết</a>
                                    ...
                                    <button type="button" onclick="getId('')" class="btn btn-default btn-xs mbs" data-target="#delete-modal" data-toggle="modal"><i class="fa fa-trash-o"></i>&nbsp;
                                        Xóa</button>
                                    @endif
                                    @if(canEdit($tt->trangthaihoso))
                                        @if($tt->trangthaihoso == 'Chờ duyệt')
                                            <button type="button" onclick="confirmTraLai()" class="btn btn-default btn-xs mbs" data-target="#tralai-modal" data-toggle="modal"><i class="fa fa-reply"></i>&nbsp;
                                                Trả lại</button>
                                        @endif
                                    @endif
                                    @if(canEdit($tt->trangthaihoso))
                                        @if($tt->trangthaihoso == 'Chờ duyệt')
                                            <button type="button" onclick="confirmNhanHs()" class="btn btn-default btn-xs mbs" data-target="#nhanhs-modal" data-toggle="modal"><i class="fa fa-check"></i>&nbsp;
                                               Duyệt</button>
                                        @endif
                                    @endif
                                </td>
        ...
        <div class="modal fade" id="delete-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    {!! Form::open(['url'=>'doituongtx/delete','id' => 'frm_delete'])!!}
